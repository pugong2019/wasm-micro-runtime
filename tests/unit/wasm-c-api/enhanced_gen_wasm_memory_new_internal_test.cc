...
// Test Case 1: Test wasm_memory_new_internal through module instantiation with memory
TEST_F(WasmMemoryNewInternalTest, ModuleWithMemory_InstantiationCreatesMemory_SucceedsCorrectly)
{
    // Load module with memory
    wasm_module_t *module = load_module_from_bytes(wasm_memory_module, sizeof(wasm_memory_module));
    if (!module) {
    ...
    // Verify memory exists (even if not exported, it should be created internally)
    // The test validates that wasm_memory_new_internal was called successfully
...
TEST_F(WasmMemoryNewInternalTest, ModuleWithoutMemory_InstantiationHandlesCorrectly_SucceedsGracefully)
{
    // Load module without memory
    wasm_module_t *module = load_module_from_bytes(wasm_no_memory_module, sizeof(wasm_no_memory_module));
...
TEST_F(WasmMemoryNewInternalTest, ValidParameters_MemoryCreation_SucceedsCorrectly)
{
    // Create memory limits
    wasm_limits_t limits = { 1, 10 };  // 1 page min, 10 pages max
    wasm_memorytype_t *memory_type = wasm_memorytype_new(&limits);
    ...
    const wasm_limits_t *retrieved_limits = wasm_memorytype_limits(retrieved_type);
...
TEST_F(WasmMemoryNewInternalTest, BoundaryConditions_MemoryCreation_HandlesCorrectly)
{
...
TEST_F(WasmMemoryNewInternalTest, MultipleMemoryCreation_IndependentObjects_CreatedCorrectly)
{
...
TEST_F(WasmMemoryNewInternalTest, ModuleInstantiation_MemoryCreationPaths_CoverInternalFunction)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // Verify instance was created successfully (indicating memory creation succeeded)
        wasm_extern_vec_t exports;
        wasm_instance_exports(instance, &exports);
        
        // The fact that we can get exports means the instance was created successfully
        // which means wasm_memory_new_internal was called and succeeded
...
TEST_F(WasmMemoryNewInternalTest, ParameterValidation_ComprehensiveTest_CoversAllPaths)
{
