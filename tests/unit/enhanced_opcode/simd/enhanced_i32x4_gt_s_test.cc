/**
 * Enhanced unit tests for i32x4.gt_s WASM SIMD opcode
 * Tests signed greater-than comparison on 4 x 32-bit integer lanes
 */
...
/**
 * @class I32x4GtSTest
 * @brief Comprehensive test suite for i32x4.gt_s SIMD opcode
 * @details Tests signed greater-than comparison operation across interpreter and AOT modes.
 *          Validates proper SIMD lane-wise comparison with signed 32-bit integer semantics.
 */
class I32x4GtSTest : public testing::TestWithParam<RunningMode> {
protected:
    /**
    ...
    /**
     * @brief Load WASM test module containing i32x4.gt_s test functions
     * @details Reads WASM bytecode file and validates successful module loading
     */
    void LoadTestModule() {
        const char* wasm_file = "wasm-apps/i32x4_gt_s_test.wasm";
     ...
     */
    void CallI32x4GtSFunction(const char* func_name, const int32_t* a_lanes,
 ...
 * @brief Validates fundamental i32x4.gt_s functionality with typical signed integer values
 ...
 * @coverage_target core/iwasm/interpreter/wasm_interp_classic.c:simd_i32x4_gt_s_operation
 ...
 */
TEST_P(I32x4GtSTest, BasicComparison_ReturnsCorrectMasks) {
    // Test case 1: Mixed positive/negative comparisons
    int32_t a1[] = {5, -3, 8, -15};
    int32_t b1[] = {1, -5, 10, -20};
    int32_t result1[4];
    int32_t expected1[] = {-1, -1, 0, -1}; // 0xFFFFFFFF = -1, 0x00000000 = 0

    CallI32x4GtSFunction("test_i32x4_gt_s_basic", a1, b1, result1);

    ASSERT_EQ(expected1[0], result1[0]) << "Lane 0: 5 > 1 should be true";
    ASSERT_EQ(expected1[1], result1[1]) << "Lane 1: -3 > -5 should be true";
    ASSERT_EQ(expected1[2], result1[2]) << "Lane 2: 8 > 10 should be false";
    ASSERT_EQ(expected1[3], result1[3]) << "Lane 3: -15 > -20 should be true";
    ...
    CallI32x4GtSFunction("test_i32x4_gt_s_basic", a2, b2, result2);

    for (int i = 0; i < 4; ++i) {
        ASSERT_EQ(expected2[i], result2[i]) << "Lane " << i << " should be true (all greater)";
    }
    ...
    CallI32x4GtSFunction("test_i32x4_gt_s_basic", a3, b3, result3);

    for (int i = 0; i < 4; ++i) {
        ASSERT_EQ(expected3[i], result3[i]) << "Lane " << i << " should be false (all lesser)";
    }
}
 ...
 *          INT32_MAX > INT32_MIN and proper handling of extreme values.
 ...
 */
TEST_P(I32x4GtSTest, BoundaryValues_HandlesExtremeIntegers) {
    // Test case 1: Integer boundary extremes
    int32_t a1[] = {INT32_MAX, INT32_MIN, -1, INT32_MAX};
    int32_t b1[] = {INT32_MIN, INT32_MAX, 0, 0};
    int32_t result1[4];
    int32_t expected1[] = {-1, 0, 0, -1}; // T,F,F,T

    CallI32x4GtSFunction("test_i32x4_gt_s_boundary", a1, b1, result1);

    ASSERT_EQ(expected1[0], result1[0]) << "INT32_MAX > INT32_MIN should be true";
    ASSERT_EQ(expected1[1], result1[1]) << "INT32_MIN > INT32_MAX should be false";
    ASSERT_EQ(expected1[2], result1[2]) << "-1 > 0 should be false";
    ASSERT_EQ(expected1[3], result1[3]) << "INT32_MAX > 0 should be true";

    // Test case 2: Zero boundary comparisons
    int32_t a2[] = {0, 1, -1, -1};
    int32_t b2[] = {-1, 0, 0, -1};
    int32_t result2[4];
    int32_t expected2[] = {-1, -1, 0, 0}; // T,T,F,F

    CallI32x4GtSFunction("test_i32x4_gt_s_boundary", a2, b2, result2);

    ASSERT_EQ(expected2[0], result2[0]) << "0 > -1 should be true";
    ASSERT_EQ(expected2[1], result2[1]) << "1 > 0 should be true";
 ...
 *          mathematical identity properties of signed greater-than comparison.
 ...
 */
TEST_P(I32x4GtSTest, ZeroOperations_ProducesCorrectIdentityResults) {
    ...
    CallI32x4GtSFunction("test_i32x4_gt_s_zero", a1, b1, result1);
    ...
    CallI32x4GtSFunction("test_i32x4_gt_s_zero", a2, b2, result2);
    ...
    int32_t a3[] = {3, -5, 50, -100};
    int32_t b3[] = {5, -3, 100, -200};
    int32_t result3[4];
    int32_t expected3[] = {0, 0, 0, -1}; // F,F,F,T

    CallI32x4GtSFunction("test_i32x4_gt_s_zero", a3, b3, result3);

    ASSERT_EQ(expected3[0], result3[0]) << "3 > 5 should be false";
    ASSERT_EQ(expected3[1], result3[1]) << "-5 > -3 should be false";
    ASSERT_EQ(expected3[2], result3[2]) << "50 > 100 should be false";
    ASSERT_EQ(expected3[3], result3[3]) << "-100 > -200 should be true";
}
 ...
 */
TEST_P(I32x4GtSTest, SignedVsUnsigned_EnsuresSignedInterpretation) {
    // Test critical signed vs unsigned distinction values
    int32_t a[] = {static_cast<int32_t>(0x7FFFFFFF), static_cast<int32_t>(0x80000000),
                   static_cast<int32_t>(0xFFFFFFFF), static_cast<int32_t>(0x7FFFFFFF)};
    int32_t b[] = {static_cast<int32_t>(0x80000000), static_cast<int32_t>(0xFFFFFFFF),
                   static_cast<int32_t>(0x7FFFFFFF), static_cast<int32_t>(0x80000001)};
    int32_t result[4];
    int32_t expected[] = {-1, 0, 0, -1}; // T,F,F,T in signed interpretation

    CallI32x4GtSFunction("test_i32x4_gt_s_signed", a, b, result);

    ASSERT_EQ(expected[0], result[0]) << "INT32_MAX > INT32_MIN should be true (signed)";
    ASSERT_EQ(expected[1], result[1]) << "INT32_MIN > -1 should be false (signed)";
    ASSERT_EQ(expected[2], result[2]) << "-1 > INT32_MAX should be false (signed)";
    ASSERT_EQ(expected[3], result[3]) << "INT32_MAX > INT32_MIN+1 should be true (signed)";

    // Verify these would be different in unsigned comparison
    // In unsigned: 0x7FFFFFFF < 0x80000000, 0x80000000 < 0xFFFFFFFF
    // But in signed: INT32_MAX > INT32_MIN, INT32_MIN < -1
}
 ...
 * @input_conditions Valid WASM module with i32x4.gt_s instruction
 ...
 */
TEST_P(I32x4GtSTest, ModuleLoading_ValidatesWASMIntegrity) {
    ...
    wasm_function_inst_t func_basic = wasm_runtime_lookup_function(module_inst, "test_i32x4_gt_s_basic");
    ASSERT_NE(nullptr, func_basic) << "test_i32x4_gt_s_basic function should exist";

    wasm_function_inst_t func_boundary = wasm_runtime_lookup_function(module_inst, "test_i32x4_gt_s_boundary");
    ASSERT_NE(nullptr, func_boundary) << "test_i32x4_gt_s_boundary function should exist";

    wasm_function_inst_t func_zero = wasm_runtime_lookup_function(module_inst, "test_i32x4_gt_s_zero");
    ASSERT_NE(nullptr, func_zero) << "test_i32x4_gt_s_zero function should exist";

    wasm_function_inst_t func_signed = wasm_runtime_lookup_function(module_inst, "test_i32x4_gt_s_signed");
    ASSERT_NE(nullptr, func_signed) << "test_i32x4_gt_s_signed function should exist";
}
