...
/**
 * @test SubnormalValues_PreservesAccuracy
 * @brief Validates f32.const preserves subnormal/denormalized f32 values
 * @details Tests the smallest representable positive and negative subnormal values to ensure
 *          f32.const correctly handles denormalized numbers near zero.
 * @test_category Corner - Subnormal value validation
 * @coverage_target core/iwasm/interpreter/wasm_interp_classic.c:f32_const_operation
 * @input_conditions Smallest positive/negative subnormal f32 values
 * @expected_behavior Exact preservation of subnormal values without underflow to zero
 * @validation_method Bitwise comparison for subnormal value preservation
 */
TEST_P(F32ConstTest, SubnormalValues_PreservesAccuracy)
{
    // Test smallest positive subnormal f32 value
    float min_subnormal = 1.401298e-45f;  // 0x00000001 in hex
    ASSERT_TRUE(are_f32_bitwise_equal(call_const_func("get_min_subnormal"), min_subnormal))
        << "f32.const failed to preserve smallest positive subnormal value";

    // Test smallest negative subnormal f32 value
    float min_subnormal_neg = -1.401298e-45f;  // 0x80000001 in hex
    ASSERT_TRUE(are_f32_bitwise_equal(call_const_func("get_min_subnormal_neg"), min_subnormal_neg))
        << "f32.const failed to preserve smallest negative subnormal value";
}
...
/**
 * @test BitPatternPreservation_MaintainsEncoding
 * @brief Validates f32.const maintains exact bit-level representation
 * @details Tests specific bit patterns including different NaN encodings to ensure
 *          f32.const preserves exact IEEE 754 bit representations without modification.
 * @test_category Edge - Bit pattern preservation validation
 * @coverage_target core/iwasm/interpreter/wasm_interp_classic.c:f32_const_operation
 * @input_conditions Specific f32 bit patterns, various NaN encodings
 * @expected_behavior Bit-for-bit identical preservation of input patterns
 * @validation_method Bitwise comparison of input and output bit patterns
 */
TEST_P(F32ConstTest, BitPatternPreservation_MaintainsEncoding)
{
    // Test specific bit pattern preservation for various values
    float test_value = 1.23456789f;
    uint32_t original_bits = get_f32_bits(test_value);
    uint32_t result_bits = get_f32_bits(call_const_func("get_specific_pattern"));
    ASSERT_EQ(result_bits, original_bits)
        << "f32.const failed to preserve exact bit pattern for 1.23456789f";

    // Test signaling NaN bit pattern (if supported)
    float signaling_nan = call_const_func("get_signaling_nan");
    ASSERT_TRUE(std::isnan(signaling_nan))
        << "f32.const failed to preserve signaling NaN";

    // Test quiet NaN bit pattern
    float quiet_nan = call_const_func("get_quiet_nan");
    ASSERT_TRUE(std::isnan(quiet_nan))
        << "f32.const failed to preserve quiet NaN";
}
