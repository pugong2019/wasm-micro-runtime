...
For comprehensive feature testing, **a single feature may require extensive test coverage with many test cases (potentially 30, 50, or even 100+ cases)**. To manage LLM code generation complexity and maintain quality, implement **Multi-Step Feature Segmentation**:

**Key Principle**: **Reduce LLM single-time code generation load, NOT limit total feature test cases**

**Segmentation Formula**:
- **Gap-Based Planning**: Test case count determined by coverage gap analysis (current vs target coverage)
...
- **Small Gap**: Current 80% → Target 90% = 15 test cases → 1 step (15 cases)
- **Medium Gap**: Current 60% → Target 80% = 35 test cases → 2 steps (20 + 15 cases)
- **Large Gap**: Current 40% → Target 75% = 80 test cases → 4 steps (20 + 20 + 20 + 20 cases)
- **Major Gap**: Current 20% → Target 70% = 150 test cases → 8 steps (20 × 7 + 10 cases)
- **Maximum Module**: Up to 200 test cases → 10 steps (20 × 10 cases)

**Purpose**: Each step generates ≤20 test cases to keep LLM output manageable while allowing comprehensive feature coverage across multiple steps.
...
1. **Feature Analysis**: Estimate total test cases needed for comprehensive coverage (can be 50, 100+ cases for complex features)
...
6. **Scale Management**: For features requiring >60 cases, create additional specialized steps (e.g., Step 4: Edge Cases, Step 5: Performance, Step 6: Platform-Specific)
